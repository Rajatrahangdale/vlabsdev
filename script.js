"use strict";
class AnimalTable {
    constructor(animals, tableId, sortableFields) {
      ...
      <table class="table table-bordered">
        <thead>
          <tr>
            ...
            .map((field) => `<th class=${this.sortableFields.includes(field.toLowerCase()) && "pointer"} onclick="animalTables['${this.tableId}'].sortTable('${field.toLowerCase()}')">
                    ${field} ${this.sortableFields.includes(field.toLowerCase()) ? "&#x25B2;&#x25BC;" : ""}
                  </th>`)
            .join("")}
          </tr>
        </thead>
    ...
    getFormData() {
        return {
            name: document.getElementById(`${this.tableId}-name`).value.trim(),
            image: document.getElementById(`${this.tableId}-image`).value.trim(),
            location: document.getElementById(`${this.tableId}-location`).value.trim(),
            size: Number(document.getElementById(`${this.tableId}-size`).value.trim()),
            weight: Number(document.getElementById(`${this.tableId}-weight`).value.trim()),
        };
    }
    fillForm(animal) {
        document.getElementById(`${this.tableId}-name`).value = animal.name;
        document.getElementById(`${this.tableId}-image`).value = animal.image;
        document.getElementById(`${this.tableId}-location`).value = animal.location;
        document.getElementById(`${this.tableId}-size`).value = animal.size.toString();
        document.getElementById(`${this.tableId}-weight`).value = animal.weight.toString();
    }
}
const bigCats = [
